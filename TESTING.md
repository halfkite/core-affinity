...
结果：中文帮助、英文帮助、动态语言菜单、CPU 列表、P/E 标注、核心分组命令和一键禁用 E 核均成功。帮助日志显示可点击指令与 `#` 后的灰色说明；指令使用可点击补全事件。列表行会按本机最长核心标签自动补空格，例如 `0P :[✔][✘][?]`、`10P:[✔][✘][?]`，当前分组按钮显示选中颜色，其余按钮保持灰色；快捷操作按 P/E 批量设置，最后使用“应用更改”提交。服务端随后正常执行 `stop`，退出码为 0。冒烟脚本为 [coreaffinity-command-smoke.ps1](D:/ai/dxh/test-runs/coreaffinity-command-smoke.ps1)。
...
最新可安装归档：[mod-builds/20260921-2255](D:/ai/dxh/mod-builds/20260921-2255)，目录时间戳精确到分钟；SHA-256：`129A86D30C1557E74CB6FBDEED0190219A253DE20CDCC8DC80B8A823174607AA`。
